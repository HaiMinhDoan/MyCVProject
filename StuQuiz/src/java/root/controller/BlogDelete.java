 ...
 * @author user
 */
@WebServlet(name = "BlogDelete", urlPatterns = {"/BlogDelete"})
public class BlogDelete extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (user != null && (user.getRoleLevel() == 1 && user.getRoleLevel() == 3)) {
            String postId = request.getParameter("bid");
            try {
                new PostDAO().deleteById(postId);
                response.sendRedirect("BlogManage");
            } catch (Exception e) {
                throw new ServletException("Error retrieving post for update", e);
            }
        } else {
