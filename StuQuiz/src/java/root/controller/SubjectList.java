...
public class SubjectList extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        try {
            request.setAttribute("allSubject", new SubjectDAO().getAll());
            User user = (User) request.getSession().getAttribute("userAuthorization");
            if (user != null&&(user.getRoleLevel()==3L||user.getRoleLevel()==1L)) {
                if(user.getRoleLevel()==3L){
                    request.setAttribute("allSubject", new SubjectDAO().getListSubjectByTeacherId(user.getUserId()+""));
                }
