    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (user != null && user.getRoleLevel() == 3) {
            try {
                ...
                List<SubjectStatistic> listSubject = statisticDao.getTeacherTopRevenue(user.getUserId() + "");
                ...
                double totalRevenue = statisticDao.teacherTotalRevenue(user.getUserId() + "");
                int totalView = statisticDao.teacherTotalView(user.getUserId() + "");
                String formattedTotalView = ViewFormatter.formatViews(totalView);
                double monthlySales = itemDao.getTeacherMonthlySales(user.getUserId() + "");

                if (actionCheck != null && (actionCheck.equals("sort") || actionCheck.equals("viewAll"))) {
                    listSubject = statisticDao.getAllSubjectsOfOwner(user.getUserId() + "");
                    ...
                    listSubject = statisticDao.getTeacherStatisticBySubjectName(searchText.toLowerCase().trim(), user.getUserId() + "");
                    subStatistic = new ArrayList<>();
                    for (SubjectStatistic s : listSubject) {
                        Subject subjectTemp = subjectDao.getById(String.valueOf(s.getSubjectId()));
                        if (subjectTemp != null) {
